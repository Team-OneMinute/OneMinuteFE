...
export class ResultBoard {
    ...
    // constructor
    constructor(canvas: HTMLCanvasElement, backAction: () => void, retryAction: () => void) {
        this.canvas = canvas;
        ...
        this.backButton.setPosition('margin', {
            right: canvas.width / 2 + 10,
            bottom: this.canvas.height - (this.rectangle.top + this.rectangle.height) + 20,
        });
        this.retryButton = new Button(canvas, 0, 0, 70, 35, retryAction, 'retry');
        this.retryButton.setPosition('margin', {
