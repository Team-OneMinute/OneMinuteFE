  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const update = () => {
        ...
        if (!block.isTouch) {
          block.x = (Math.sin(Date.now() / 1000 * 2) * canvas.width / 4) + (canvas.width / 2 - block.width / 2);
        }
        ...
        const isStrikeTableFromOn = inTableX && prevCharacterY + character.height <= table.y && isTouchObject(nextCharacterY, table.y, table.height);
        const isStrikeBlockFromOn = inBlockX && prevCharacterY + character.height <= block.y && isTouchObject(nextCharacterY, block.y, block.height);
        const isStrikeTableFromUnder = inTableX && prevCharacterY >= table.y + table.height && isTouchObject(nextCharacterY, table.y, table.height);
        const isStrikeBlockFromUnder = inBlockX && prevCharacterY >= block.y + block.height && isTouchObject(nextCharacterY, block.y, block.height);
        if ((prevCharacterY + character.height <= groundY && nextCharacterY + character.height > groundY) || isStrikeTableFromOn || isStrikeBlockFromOn) {
          character.vy = 0;
